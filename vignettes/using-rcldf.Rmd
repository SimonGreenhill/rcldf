...
[Grambank](https://grambank.clld.org)), phonetic information (e.g.
...
# and finally, the lexical items (forms) themselves:
head(ds$tables$FormTable)

# and the cognate judgements:
head(ds$tables$CognateTable)
```

Note: the table name depends on the dataset type. `ValueTable` is used in
**StructureDatasets** (e.g. WALS, Grambank), whereas **Wordlists** store lexical
items in `FormTable` and cognacy information in `CognateTable`. If
`ds$tables$ValueTable` returns `NULL`, check `ds$type` and `names(ds$tables)` to
...
Unlike `read.csv()`, `get_table_from` (and `cldf()` generally) gives you three
things for free:

- **Correct column types** — column types (integer, date, boolean, …) are read
  from the dataset's `tableSchema`, not guessed from the data.
- **Proper `NA` values** — each CLDF dataset declares its own null markers
  (e.g. `""`, `"?"`) in the metadata; these are automatically converted to R
  `NA`.
- **Stable table names** — you refer to tables by their CLDF role
  (`"LanguageTable"`) rather than the underlying filename (`languages.csv`,
  `Languages.csv`, …), which differs across datasets.
...
Glottolog, Concepticon, CLTS, and D-PLACE CLDF reference catalogues:
...
`rcldf` has functions to find other datasets and load them easily using the CLDF-META project:

```{r}
datasets()
```

And too easily install one, find the Dataset name:
...
However, by specifying a persistent directory (e.g.
...
```{r, eval=FALSE}
url <- "https://zenodo.org/record/7844558/files/grambank/grambank-v1.0.3.zip?download=1"
df <- cldf(url, cache_dir = "~/data/grambank")
```
...
To see what languages we have, let's use `rcldf`'s plotting functionality:
...
```{r}
# using 'select' here to make it easier to see
