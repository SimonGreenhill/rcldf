...
test_that("test summary.cldf", {
    ...
    expect_match(out[7], "1/4: codes \\(4 columns, 5 rows\\)")
    expect_match(out[8], "2/4: languages \\(9 columns, 9 rows\\)")
    expect_match(out[9], "3/4: parameters \\(6 columns, 1 rows\\)")
    expect_match(out[10], "4/4: values \\(7 columns, 9 rows\\)")
