...
test_that("test nullify", {
    ...
    # No changes
    cldfobj_null <- nullify(cldfobj, nulls)
    for (t in names(cldfobj$tables)) {
        expect_equal(cldfobj$tables[[t]], cldfobj_null$tables[[t]])
    }
    ...
    cldfobj_null <- nullify(cldfobj, nulls)
    # no changes here
    expect_equal(cldfobj$tables[["ParameterTable"]], cldfobj_null$tables[["ParameterTable"]])
    expect_equal(cldfobj$tables[["CodeTable"]], cldfobj_null$tables[["CodeTable"]])

    # and no changes in these columns...
    for (col in c("ID", "Language_ID", "Parameter_ID", "Code_ID", "Comment", "Source")) {
        expect_equal(cldfobj$tables[["ValueTable"]][[col]], cldfobj_null$tables[["ValueTable"]][[col]])
    }
    ...
    # changes here
    expect_equal(
        cldfobj_null$tables[["ValueTable"]][["Value"]],
        c(NA, "5", "1", NA, "5", NA, "3", "4", "3")
    )
    expect_equal(
        cldfobj_null$tables[["LanguageTable"]][["Family"]],
